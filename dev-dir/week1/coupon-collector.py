...
def export_cdf_values(args: argparse.Namespace) -> None:
    trials = simulation(args.n, args.m)
    np.savetxt('data/cc-data/trials.csv', trials, delimiter=',', fmt='%d')
...
def run_benchmark(args: argparse.Namespace) -> None:
    ns = np.linspace(1_000, 20_000, args.n, dtype=int)
    ms = np.linspace(500, 5_000, args.m, dtype=int)
    ...
    np.savetxt('data/cc-data/bench.csv', results, delimiter=',', header='n,m,t', comments='', fmt=['%d','%d','%.4f'])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog='coupon-collector', description='coupon collector simulation')

    subparsers = parser.add_subparsers(dest='command')

    for cmd in ['simulate', 'expectation', 'get-cdf-vals', 'duration']:
        add_common_args(subparsers.add_parser(cmd, description=f'runs {cmd} command'))
